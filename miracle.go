...
//
// Time: O(∞). Space: O(1).
type MiracleSorter[T cmp.Ordered] struct {
...
func (MiracleSorter[T]) Name() string { return "Miracle Sort" }
...
func (m MiracleSorter[T]) Sort(input []T) []T {
	...
	for checks := 0; !slices.IsSorted(arr); checks++ {
		if m.MaxChecks > 0 && checks >= m.MaxChecks {
			break
		}
		// Wait for a miracle (cosmic ray flipping a bit in memory). In practice
		// this never happens, so we just check again.
	}
	return arr
}
